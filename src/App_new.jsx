...
function App() {
  ...
  // 偵錯版 handleModalInputChange
  // const handleModalInputChange = (e) => {
  //   const { name, value, checked, type } = e.target;
  //   const newValue = type === 'checkbox' ? checked : value;

  //   console.log(`--- 2. handleModalInputChange 觸發 ---`);
  //   console.log(`欄位 (name): ${name}, 新值 (newValue):`, newValue);
  ...
  //       [name]: newValue,
  //     };
  //     console.log('更新後的 templateProduct 狀態:', updatedData);
  //     return updatedData;
  //   });
  // }
  ...
  const updateProduct = async () => {
    ...
    if (modalType === "edit") {
      ...
      if (typeof templateProduct.id !== 'string' || !templateProduct.id) {
  ...
  const openModal = (type, product) => {
    ...
    setModalType(type);
    if (type === 'create') {
    // 當建立新產品時，永遠使用最乾淨的初始樣板
    setTemplateProduct(BASIC_TEMPLATE_DATA);
  } else {
    // 當編輯時，才使用傳入的 product 資料
    setTemplateProduct({
      ...product, // 展開所有從 API 來的資料
      is_enabled: Boolean(product.is_enabled), // 確保 is_enabled 永遠是布林值
